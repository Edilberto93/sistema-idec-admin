...
//codigo iglesia para pagos
// Tu función sigue intacta y funcionando
async function cargarIglesiasEnSelect(selectId) {
    const select = document.getElementById(selectId);
    if (!select) return; 
...
// Inicialización: Aquí activamos los dos selectores que tienes
document.addEventListener("DOMContentLoaded", () => {
    // Carga el select de pagos
    cargarIglesiasEnSelect('codigoIglesiaPago'); 
    
    // Carga el nuevo select de filtro
